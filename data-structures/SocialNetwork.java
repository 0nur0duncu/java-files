...
public class SocialNetwork {
   ...
   */
  public List<String> mostPopular1() {
    
    List<String> keys;
    keys = new ArrayList<>(adjacencyList.keySet());
    ...
    for(int i=0;i < adjacencyList.size()-1;i++){
      ...
      for(int j=i+1;j < adjacencyList.size();j++)
        if(values.get(j) > values.get(i)) max = j;
      String temp_k = keys.get(i);
      Integer temp_v = values.get(i);
  ...
  public List<String> mostCommonFriends() {
    ...
    int hold = 0;
    for(int i=0;i<adjacencyList.size()-1;i++){
        List<String> closeFriends1 = new ArrayList<>();
        for(String friends1:adjacencyList.get(profiles.get(i)))
            if(!closeFriends1.contains(friends1))
                closeFriends1.add(friends1);
        for(int j=i+1;j<adjacencyList.size();j++){
            int counter = 0;
            List<String> closeFriends2 = new ArrayList<>();
            for(String friends2:adjacencyList.get(profiles.get(j)))
            if(!closeFriends2.contains(friends2)){
                closeFriends2.add(friends2);
                if(closeFriends1.contains(friends2)) counter++;
            }
            if(counter>hold){
                willReturn.clear();
